...
| **TD-019: Frontend ESLint Cleanup** | High | 📋 Sprint 10 | **Issue:** Frontend ESLint has 49 errors + 21,363 warnings. CI pipeline does NOT run frontend lint. 21,354 warnings are CRLF prettier (no .gitattributes on Windows). 49 errors: React Compiler (24), prettier (13), no-explicit-any (9), no-unused-vars (9), react-refresh (3), jsx-a11y (4). **Plan:** Add .gitattributes, `eslint --fix` for CRLF, fix 49 errors manually, add `npm run lint` to CI frontend job with `--max-warnings=0`. **Effort:** 3-4h. **Discovered:** During Story 10.3 development. |
