...
### Success Criteria
- ✅ Authorized users (ADMIN/ISSUER) can issue individual badges
- ✅ Batch issuance via CSV upload works for 10+ badges
- ✅ Recipients receive email notifications with claim links
- ✅ Recipients can claim badges via unique token
- ✅ Issued badges include Open Badges 2.0 compliant assertion
- ✅ Public verification endpoint returns valid assertions
- ✅ Badge revocation workflow functions correctly
- ✅ All tests pass (unit + E2E) with 100% pass rate
...
## 📚 Key Lessons Applied (Sprint 0-2)

### From Sprint 2 Retrospective

#### ✅ Lesson 10: Code-Documentation Sync
...
#### ✅ Lesson 11: Adjusted Time Estimates
...
#### ✅ Lesson 13: Prisma JSON Types
**Action:** Badge.assertionJson will need DTO → Plain Object conversion
```typescript
// When saving assertion to database:
assertionJson: JSON.parse(JSON.stringify(assertionDto))
```

#### ✅ Lesson 14: Union Type Validation
...
├── badge-issuance.service.ts         # Core business logic
├── badge-issuance.module.ts
├── dto/
│   ├── issue-badge.dto.ts            # Single issuance
│   ├── bulk-issue-badges.dto.ts      # CSV upload
│   ├── claim-badge.dto.ts            # Claim token
│   ├── revoke-badge.dto.ts           # Revocation
│   └── query-badge.dto.ts            # Filtering
└── services/
    ├── assertion-generator.service.ts # OB 2.0 JSON-LD
    ├── csv-parser.service.ts          # Bulk issuance
    └── badge-verification.service.ts  # Public verification
```
...
**Mitigation:**
- ✅ **Option A (Recommended):** Start domain verification NOW (before Sprint 3 dev starts)
- ✅ **Option B:** Use Azure-provided domain for MVP (instant, but ugly sender address)
- ✅ **Option C:** Keep Ethereal for development, deploy ACS later
...
- AssertionGeneratorService (8 tests)
  - ✅ Generate valid OB 2.0 JSON-LD
  - ✅ Hash recipient email (SHA-256)
...
- Issue Badge (3 tests)
  - ✅ Authorized user can issue
  - ✅ Unauthorized user cannot issue
  - ✅ Invalid template ID rejected
  
- Bulk Issue (3 tests)
  - ✅ Valid CSV processes successfully
  - ✅ Invalid CSV rejected with errors
  - ✅ Partial failures handled
  
- Claim Badge (3 tests)
  - ✅ Valid token claims successfully
  - ✅ Invalid token rejected
  - ✅ Expired token rejected
  
- Query Badges (3 tests)
  - ✅ User sees own badges only
  - ✅ Issuer sees issued badges
  - ✅ Admin sees all badges
  
- Revoke Badge (3 tests)
  - ✅ Admin can revoke
  - ✅ Non-admin cannot revoke
  - ✅ Revoked badge returns 410 Gone

#### E2E Tests (PowerShell) - 5 tests
- ✅ Complete issuance workflow (issue → notify → claim)
- ✅ Bulk issuance smoke test
- ✅ Public assertion endpoint
- ✅ Badge verification
- ✅ Revocation workflow
...
- ✅ Story 4.5: Email Notifications (2h)

**Afternoon (3h):**
- ✅ Story 4.3: Badge Claiming Workflow (2h)
- ✅ Setup Azure Communication Services (1h)

### Day 2 (5-6 hours)
**Morning (3-4h):**
- ✅ Story 4.2: Batch Badge Issuance (CSV) (3h)
- ✅ Story 4.4: Issuance History & Queries (2h)

**Afternoon (2h):**
- ✅ Story 4.6: Badge Revocation (1.5h)
- ✅ Final testing & bug fixes (30min)

### Day 3 (Optional, 2-3 hours)
- ✅ Sprint retrospective
- ✅ Update documentation
- ✅ Prepare Sprint 3 final report
