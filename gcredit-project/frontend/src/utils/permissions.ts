/**
 * permissions.ts — Frontend permission computation
 *
 * Pure functions to compute dashboard tabs, sidebar groups, and flat
 * permission booleans from JWT claims (role + isManager).
 *
 * CROSS-001: This utility MUST produce identical results to the backend
 * `compute-permissions.ts` for the same (role, isManager) inputs.
 * Backend canonical source: backend/src/modules/auth/utils/compute-permissions.ts
 *
 ...
 * @see Story 15.2 — Backend Permissions API
 */

export type UserRole = 'EMPLOYEE' | 'ISSUER' | 'ADMIN';
 ...
 */
export function computeFlatPermissions(role: UserRole, isManager: boolean): FlatPermissions {
  return {
 ...
 */
export function computePermissions(role: UserRole, isManager: boolean): ComputedPermissions {
  return {
    role,
    isManager,
