...
| FR10 | Enforce role-based issuing permissions with RBAC | ✅ **Complete** | 4-role RBAC (ADMIN, ISSUER, MANAGER, EMPLOYEE). `RolesGuard` + `@Roles()` decorator across all controllers. Badge issuance restricted to ADMIN/ISSUER. Revocation restricted to ADMIN/ISSUER/MANAGER. | Registration no longer allows role self-assignment (SEC-HIGH-003 fixed Sprint 8). |
