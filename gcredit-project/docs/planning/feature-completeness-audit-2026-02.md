...
| 5 | View issued badges → Search → Filter → Sort → Revoke | ✅ | `BadgeManagementPage.tsx` — Multi-filter search, status tabs, revocation modal with reason/notes. Role-scoped: Admin sees all, Issuer sees own, Manager sees dept. **[UAT]** Sortable columns (3-click cycle), auto-paginated full-data fetch, `table-fixed` layout | No link from badge row to badge detail view |
| 6 | View analytics → Export data | ✅ | `AdminAnalyticsPage.tsx` — KPI cards, issuance trends chart, top performers, skills distribution, recent activity. **[Sprint 11]** CSV export added (11.23). **[UAT]** ISSUER role-scoped overview (own-issued badges only) | — |
| 7 | Manage users → Assign roles | ✅ | `AdminUserManagementPage.tsx` — Search, filter by role/status, pagination, role change, activate/deactivate, change department. **[UAT]** Search extended to match role & department; Department/Status columns sortable (3-click cycle) | No create-user UI (only PATCH operations) |
...
| 2 | See badge details, issuer, criteria | ✅ | Badge image, name, description, issuer, dates, criteria, skills, evidence links, Open Badges 2.0 JSON-LD download | **[Sprint 11]** Skills now show `nameEn` (11.18) |
...
| 6 | Badge Detail Modal | (Modal overlay) | ✅ Yes | ⚠️ Partial | Full content per spec. **[UAT]** Hero now shows "Issued by" name. **Gaps:** Footer uses inline `style={{}}`, no focus management on open, `z-[9999]` hardcoded |
...
| 12 | Badge Management | `/admin/badges` | ✅ Yes | ✅ Yes | Table/card views, multi-filter search, status tabs, revocation workflow. Mobile card layout. **[UAT]** Sortable columns (3-click cycle), auto-paginated full-data fetch, `table-fixed` + `colgroup` layout, mobile sort dropdown |
...
| 19 | User Management | `/admin/users` | ✅ Yes | ⚠️ Partial | Search, role/status filter, pagination, role change, activate/deactivate. **[UAT]** Search matches role & department; Department/Status sortable. **Gaps:** No create-user button, no user detail view, retry uses `window.location.reload()` |
| 20 | Analytics Dashboard | `/admin/analytics` | ✅ Yes | ⚠️ Partial | KPI cards, issuance trends, top performers, skills distribution, recent activity. **[Sprint 11]** CSV export added (11.23). **[UAT]** ISSUER role-scoped overview. **Gaps:** No auto-refresh, system health shows string not sparkline |
...
| ~~Analytics CSV export~~ | ~~P1~~ | ~~Section 6.1~~ | **Fixed** — Sprint 11 (11.23) |
...
| ~~P1-5~~ | ~~**Analytics has no data export**~~ — **Fixed in Sprint 11 (11.23):** CSV export endpoint added to analytics controller | ~~Resolved~~ |
| ~~P1-6~~ | ~~**Verification page shows raw skill UUIDs**~~ — **Fixed in Sprint 11 (11.18):** Shows `nameEn` instead of raw UUID | ~~Resolved~~ |
...
| ~~P2-3~~ | ~~`BadgeManagementPage` — Client-side filter on server-paged data (mismatch)~~ | ~~Fixed in UAT: auto-paginated fetch loads all data client-side~~ |
...
| ~~P1-5: Analytics export~~ | ~~CSV export endpoint~~ — **Done (11.23)** | — | — |
| ~~P1-6: Skill UUID resolution~~ | ~~Verification skill names~~ — **Done (11.18)** | — | — |
...
| Analytics Export | 1 (CSV export) | ✅ Added Sprint 11 (11.23) |
