...
- [ ] **Prisma Json Field?**
  - YES ‚Ü?Plan to convert DTO to plain object before saving
...
- [ ] **Union Type Field?** (e.g., `string | number | boolean`)
  - YES ‚Ü?Add validation decorator (minimum `@IsNotEmpty()`)
...
- [ ] **Nested DTO with @ValidateNested()?**
  - YES ‚Ü?Remember: This creates class instances!
...
```typescript
// ‚ú?CORRECT Pattern for DTOs with Json storage
...
## üõ£Ô∏?During Development - Route Design
...
- [ ] **Is this endpoint public?**
  - YES ‚Ü?Add `@Public()` decorator
  - NO ‚Ü?Add `@Roles(UserRole.X)` decorator
...
```typescript
// ‚ú?CORRECT - Public endpoint with global guard
...
- [ ] **DTO Validation Tests**
  - Valid nested DTO ‚Ü?Should pass
  - Invalid union type ‚Ü?Should fail with clear error
  - Missing required fields ‚Ü?Should fail

- [ ] **Prisma Integration Tests**
  - Save DTO with nested objects ‚Ü?Should succeed
  - Retrieve and verify Json field ‚Ü?Data intact
  - Test with class instances (not plain objects)

- [ ] **Route Tests**
  - Specific route before `:id` ‚Ü?Should match correctly
  - Dynamic route with UUID ‚Ü?Should work
  - Specific route returns correct data (not `:id` handler)

- [ ] **Authentication Tests**
  - Public endpoint without token ‚Ü?Should work (200)
  - Protected endpoint without token ‚Ü?Should fail (401)
  - Public endpoint with token ‚Ü?Should work (200)
...
1. **Prisma Error:** `Type 'MyDto' is not assignable to type 'JsonValue'`
   ‚Ü?Missing JSON.parse(JSON.stringify()) conversion

2. **Validation Error:** `"property value should not exist"`
   ‚Ü?Missing validator on union type field

3. **Wrong Endpoint:** Calling `/resources/templates` but `:id` handler executes
   ‚Ü?Route order incorrect, `:id` defined before `templates`

4. **Unexpected 401:** Public endpoint returns Unauthorized
   ‚Ü?Missing @Public() decorator (global guard active)

---

## üìö Quick Reference Links

- [backend-code-structure-guide.md](../backend-code-structure-guide.md) - Copy-paste imports & patterns
- [lessons-learned.md](../gcredit-project/docs/lessons-learned/lessons-learned.md) - Detailed explanations
