...
4. [x] `scope = global`: no filter applied �?counts all user's claimed badges
5. [x] `scope = category`: filters by `categoryId` (and optionally all descendant sub-categories); query chain: `SkillCategory �?Skill �?BadgeTemplate.skillIds hasSome`
...
8. [x] `MilestoneType` Prisma enum migrated: `BADGE_COUNT | SKILL_TRACK | ANNIVERSARY | CUSTOM` �?`BADGE_COUNT | CATEGORY_COUNT`
9. [x] Existing milestone data migrated: `SKILL_TRACK` �?`BADGE_COUNT` (with category scope), `ANNIVERSARY`/`CUSTOM` �?deactivated
10. [x] Active config cache retained (5-min TTL, existing behavior)

### Milestone Trigger DTO & Validation (Backend)
11. [x] `MilestoneTriggerDto` validates: `metric` (enum), `scope` (enum), `threshold` (int �?1), `categoryId` (UUID, required when scope=category), `includeSubCategories` (boolean, optional)
...
27. [x] Metric selector: radio group �?"Badge Count" / "Category Coverage"
28. [x] Scope selector: radio group �?"Global (all badges)" / "Specific Category"
...
33. [x] Admin can edit milestone �?metric and scope **locked** after creation (disabled + tooltip: "Cannot change metric/scope after creation")
34. [x] Icon selection: Lucide icon picker (curated ~20 achievement icons)

### Tests
35. [x] Engine evaluator tests: `badge_count × global`, `badge_count × category`, `category_count × global`, invalid combo rejection
...
- [x] Task 1: Refactor `MilestoneEvaluator` (AC: #1�?5, #7, #10)
  - [x] Replace `evaluateTrigger()` switch-on-type with `metric + scope` evaluation
  - [x] Implement `buildScopeFilter(trigger)`: global �?`{}`, category �?skill-chain query
...
- [x] Task 2: DTO & Validation upgrade (AC: #6, #11�?13)
...
- [x] Task 4: Dashboard integration (AC: #14�?16)
...
- [x] Task 5: Achievement notification (AC: #17�?18)
...
- [x] Task 6: Milestone management page (AC: #21�?25)
...
- [x] Task 7: Create milestone form (AC: #26�?32, #34)
...
- [x] Task 9: Toggle isActive (AC: #24)
  ...
  - [x] API: `PATCH /api/admin/milestones/:id` �?`{ isActive }`
  - [x] Sonner toast confirmation
- [x] Task 10: CelebrationModal + Timeline integration (AC: #19, #20)
  - [x] On badge claim: check `newMilestones` in response �?trigger `CelebrationModal`
  - [x] Wire `MilestoneTimelineCard` into `TimelineView`
- [x] Task 11: API integration
  - [x] `GET /api/admin/milestones` �?list with achievement counts
  - [x] `POST /api/admin/milestones` �?create
  - [x] `PATCH /api/admin/milestones/:id` �?update
  - [x] `GET /api/skill-categories` �?for category tree picker

### Tests (4h)

- [x] Task 12: Engine tests (AC: #35�?37)
  - [x] `badge_count × global` evaluator test
  - [x] `badge_count × category` evaluator test (with sub-categories)
  - [x] `category_count × global` evaluator test
  - [x] Invalid combo rejection test (`category_count × category` �?400)
  - [x] `includeSubCategories` flag test (true vs false)
  - [x] Prisma migration data integrity test
- [x] Task 13: Integration + UI tests (AC: #38�?40)
