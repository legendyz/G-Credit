...
| TD-009 | Milestone Admin UI | S11 | Medium | 2-3d | 📋 Backlog (2026-02-18) |
| TD-010 | Evidence System Unification | S11 | High | 3-5d | 📋 Backlog (2026-02-18) |

**Total Technical Debt Effort:** 40-60 hours

### TD-010: Evidence System Unification (凭证系统统一)

**背景：** 当前存在两套并行的凭证机制，导致数据模型混乱、用户体验不一致。
...
| 机制 | 存储 | 时机 | 数量 | 安全 |
|------|------|------|------|------|
| `Badge.evidenceUrl` | 字段 | 发放时 | 1 个外部 URL | 无 |
| `EvidenceFile` 表 | Azure Blob | 发放后 | ≤5 个文件 | SAS token |

#### 2. 前端展示不一致
- **发放界面** (`IssueBadgePage`): 只收集 `evidenceUrl`
- **Wallet DetailModal**: 只展示 `evidenceFiles`（可下载）
- **验证页面** (`VerifyBadgePage`): 展示 `evidenceFiles` 但使用直接 blobUrl（无 SAS token，可能无法访问）
...
3. **管理界面**
   - Badge Management 页面添加"查看详情"→ 展示凭证
   - 在详情中为 Issuer/Admin 添加"上传凭证"按钮
   
4. **验证页面修复**
   - 使用 SAS token 下载端点，而非直接 blobUrl
   - 或使用公开预签名 URL

**优先级：** High（用户体验严重不一致）
**预估工期：** 3-5 天

### TD-009: Milestone Admin UI

**背景：** 里程碑系统后端 API 已完整实现（CRUD + 自动触发），但前端缺少管理界面，目前只能通过 API 操作。

**现状：**
- ✅ 后端 API: `/api/admin/milestones` (CRUD)
- ✅ 自动触发: Badge 发放/认领后自动检测
- ✅ 用户成就展示: Wallet 时间线 + Dashboard 进度卡
- ❌ **缺少 Admin UI** 来创建/编辑/删除里程碑配置

**需求：**
1. Admin → Milestones 页面（列表、筛选、排序）
2. 创建里程碑表单（支持 3 种 trigger 类型）
3. 编辑/软删除功能
4. 预览已达成用户数量

**优先级：** Medium（功能可用但体验不佳）
**预估工期：** 2-3 天
